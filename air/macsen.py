...
df = pd.read_excel('air_temp.xlsx')
...
for stat in stations:
    stat_data = df[df['Station'] == stat]
    break_year = break_years[stat]

    #divide->before&after
    before = stat_data[stat_data['Year'] <= break_year]['Temperature'].values
    after = stat_data[stat_data['Year'] > break_year]['Temperature'].values
